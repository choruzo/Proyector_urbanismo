...
import {
  ...
  Line, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Area,
} from 'recharts'
import {
  getTendenciaObraNueva,
  getTendenciaValorSuelo,
  getTendenciaTransacciones,
} from '@/services/api'
...
const CHART_COLORS = {
  ...
  presupuesto:   '#10b981',
  valor:         '#f59e0b',
  transacciones: '#06b6d4',
}

function ChartError({ msg }: { msg: string }) {
  return (
    <div className="h-72 flex items-center justify-center text-red-400 text-sm gap-2">
...
export default function Tendencias() {
  ...
  const { data: obraNueva = [], isLoading: loadingObra, isError: errorObra } = useQuery({
    queryKey: ['tendencias-obra', annoInicio, annoFin],
    queryFn: () => getTendenciaObraNueva({ anno_inicio: annoInicio, anno_fin: annoFin }),
  })

  const { data: valorSuelo = [], isLoading: loadingValor, isError: errorValor } = useQuery({
    queryKey: ['tendencias-valor', annoInicio, annoFin],
    queryFn: () => getTendenciaValorSuelo({ anno_inicio: annoInicio, anno_fin: annoFin }),
  })

  const { data: transacciones = [], isLoading: loadingTx, isError: errorTx } = useQuery({
    queryKey: ['tendencias-tx', annoInicio, annoFin],
    queryFn: () => getTendenciaTransacciones({ anno_inicio: annoInicio, anno_fin: annoFin }),
  })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              {YEARS.map((y) => (
                <option key={y} value={y} disabled={y < annoInicio}>{y}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      <div className="card">
        ...
        {loadingObra ? (
          ...
          <ChartError msg="Error al cargar datos de obra nueva. Comprueba la conexión con el backend." />
        ) : obraNueva.length === 0 ? (
      ...
      <div className="card">
        ...
        {loadingValor ? (
          ...
          <ChartError msg="Error al cargar datos de valor del suelo." />
        ) : valorSuelo.length === 0 ? (
          <div className="h-72 flex items-center justify-center text-slate-500">
            Sin datos de valor del suelo en el rango seleccionado.
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="card">
        <h3 className="text-base font-semibold text-white mb-6">Compraventas de vivienda por año (INE)</h3>
        {loadingTx ? (
          <div className="h-72 flex items-center justify-center text-slate-500">Cargando datos...</div>
        ) : errorTx ? (
          <ChartError msg="Error al cargar datos de transacciones." />
        ) : transacciones.length === 0 ? (
          <div className="h-72 flex items-center justify-center text-slate-500">
            Sin datos de transacciones en el rango seleccionado.
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={transacciones} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis dataKey="anno" tick={{ fill: '#94a3b8', fontSize: 12 }} />
              ...
              <Tooltip
                ...
                labelStyle={{ color: '#e2e8f0', fontWeight: 600 }}
                itemStyle={{ color: '#94a3b8' }}
                formatter={(v: number) => [formatNumber(v), 'Transacciones']}
              />
              <Legend wrapperStyle={{ color: '#94a3b8', fontSize: 12 }} />
              <Bar dataKey="num_transacciones" fill={CHART_COLORS.transacciones} name="Compraventas" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
